...
c     ################################################################
c     ##                                                            ##
c     ##  subroutine chkpole1  --  check multipoles at chiral sites  ##
c     ##                                                            ##
c     ################################################################
c
c
c     "chkpole1" inverts atomic multipole moments as necessary
c     at sites with chiral local reference frame definitions
c
c
      subroutine chkpole1
...
c     loop over multipole sites testing for chirality inversion
c
!$OMP DO private(ia,ib,k,ic,id,xad,yad,zad,xbd,ybd,zbd,xcd,
!$OMP& ycd,zcd,check,c1,c2,c3,vol)
      do i = 1, npole
         check = .true.
         if (polaxe(i) .ne. 'Z-then-X')  check = .false.
         if (yaxis(i) .eq. 0)  check = .false.
