...
c     #################################################################
c     ##                                                             ##
c     ##  subroutine grid_uind1  --  put induced dipoles on PME grid  ##
c     ##                                                             ##
c     #################################################################
c
c
c     "grid_uind1" places the fractional induced dipoles onto the
c     particle mesh Ewald grid
c
c
      subroutine grid_uind1 !(fuind,fuinp)
      use sizes
      use atoms
      use chunks
      use mpole
     ...
     &                        abound(6),cbound(5),cbound(6))
               do kk = abound(5), abound(6)
                  ...
                  do jj = abound(3), abound(4)
                     ...
                     u0 = thetai2(1,m,iatm)
                     u1 = thetai2(2,m,iatm)
                     term01 = fuind_omp(2,isite)*u1*v0
     &                           + fuind_omp(3,isite)*u0*v1
                     term11 = fuind_omp(1,isite)*u0*v0
                     term02 = fuinp_omp(2,isite)*u1*v0
     &                           + fuinp_omp(3,isite)*u0*v1
                     term12 = fuinp_omp(1,isite)*u0*v0
                     do ii = abound(1), abound(2)
                        i = ii
                        m = i + offsetx
                        if (i .lt. 1)  i = i + nfft1
...
c!$OMP PARALLEL default(shared) private(i,j,k,m,ii,jj,kk,ichk,
c!$OMP& isite,iatm,cid,nearpt,cbound,abound,offsetx,offsety,
c!$OMP& offsetz,v0,v1,u0,u1,term01,term11,term02,term12,t0,t1)
...
c     ################################################################
c     ##                                                            ##
c     ##  subroutine fphi_mpole1  --  multipole potential from grid  ##
c     ##                                                            ##
c     ################################################################
c
c
c     "fphi_mpole1" extracts the permanent multipole potential from
c     the particle mesh Ewald grid
c
c
...
c     set OpenMP directives for the major loop structure
c
!$OMP DO private(iatm,igrd0,jgrd0,kgrd0,tuv000,tuv001,tuv010,
!$OMP& tuv100,tuv200,tuv020,tuv002,tuv110,tuv101,tuv011,tuv300,
...
c     set OpenMP directives for the major loop structure
c
c!$OMP PARALLEL default(private) shared(npole,ipole,igrid,bsorder,
c!$OMP& nfft3,thetai3,nfft2,thetai2,nfft1,thetai1,qgrid,fphi)
c!$OMP DO
c
c     extract the permanent multipole field at each site
c
...
c     #############################################################
c     ##                                                         ##
c     ##  subroutine fphi_uind1  --  induced potential from grid  ##
c     ##                                                         ##
c     #############################################################
c
c
...
c     #############################################################
c     ##                                                         ##
c     ##  subroutine fphi_uind2  --  induced potential from grid  ##
c     ##                                                         ##
c     #############################################################
c
c
...
c
c
      subroutine fphi_uind2 !(fdip_phi1,fdip_phi2,fdip_sum_phi)
      use sizes
      use mpole
      use pme
      use openmp
