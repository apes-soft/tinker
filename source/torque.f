...
c     ##################################################################
c     ##                                                              ##
c     ##  subroutine torque2a  --  convert all site torques to forces  ##
c     ##                                                              ##
c     ##################################################################
c
c
c     "torque2a" takes the torque values on all sites defined by
...
c
c
      subroutine torque2a !(trq,derivs)
      use sizes
      use atoms
      use mpole
      use openmp
...
c     get the local frame type and the frame-defining atoms
c
!$OMP DO schedule(dynamic,128) reduction(+:frc_omp)
      do i = 1, npole
         ia = zaxis(i)
         ib = ipole(i)
         ic = xaxis(i)
...
c     result of infinitesimal rotation along these vectors
c
         dphidu = -trq_omp(1,i)*u(1) - trq_omp(2,i)*u(2) 
     &        - trq_omp(3,i)*u(3)
         dphidv = -trq_omp(1,i)*v(1) - trq_omp(2,i)*v(2) 
     &        - trq_omp(3,i)*v(3)
         dphidw = -trq_omp(1,i)*w(1) - trq_omp(2,i)*w(2) 
     &        - trq_omp(3,i)*w(3)
         if (axetyp .eq. 'Z-Bisect') then
            dphidr = -trq_omp(1,i)*r(1) - trq_omp(2,i)*r(2) 
     &           - trq_omp(3,i)*r(3)
            dphids = -trq_omp(1,i)*s(1) - trq_omp(2,i)*s(2) 
     &           - trq_omp(3,i)*s(3)
         end if
c
c     force distribution for the Z-Only local coordinate method
c
         if (axetyp .eq. 'Z-Only') then
            do j = 1, 3
               du = uv(j)*dphidv/(usiz*uvsin) + uw(j)*dphidw/usiz
               frc_omp(j,ia) = frc_omp(j,ia) + du
               frc_omp(j,ib) = frc_omp(j,ib) - du
            end do
c
c     force distribution for the Z-then-X local coordinate method
c
            ...
            do j = 1, 3
               ...
               dv = -uv(j)*dphidu/(vsiz*uvsin)
               frc_omp(j,ia) = frc_omp(j,ia) + du
               frc_omp(j,ic) = frc_omp(j,ic) + dv
               frc_omp(j,ib) = frc_omp(j,ib) - du - dv
            end do
c
c     force distribution for the Bisector local coordinate method
c
...
c
     ...
     &                 / (wsiz*(ut1sin+ut2sin))
               frc_omp(j,ia) = frc_omp(j,ia) + du
               frc_omp(j,ic) = frc_omp(j,ic) + dv
               frc_omp(j,id) = frc_omp(j,id) + dw
               frc_omp(j,ib) = frc_omp(j,ib) - du - dv - dw
            end do
c
c     force distribution for the 3-Fold local coordinate method
c        (correct for uv, uw and vw angles all equal to 90)
