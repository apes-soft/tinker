...
c
      do k = 1, nvdw
         ...
         iv = ired(i)
         rdn = kred(i)
         xred(i) = rdn*(atom(i)%pos(1)-atom(iv)%pos(1)) 
     &        + atom(iv)%pos(1)
         yred(i) = rdn*(atom(i)%pos(2)-atom(iv)%pos(2)) 
     &        + atom(iv)%pos(2)
         zred(i) = rdn*(atom(i)%pos(3)-atom(iv)%pos(3)) 
     &        + atom(iv)%pos(3)
      end do
c
...
!$OMP DO reduction(+:evt,devt,virt) schedule(dynamic,16)
c
