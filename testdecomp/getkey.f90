...
!
!     Only process 0 reads the keys file and then broadcasts the
!     number of lines in the key file and its contents to the other
...
subroutine getkey
   ...
   if(rank.eq.0) then
      ...
      do i = 1, narg-1
         ...
         if (string(1:2) .eq. '-K') then
            ...
            if (.not. exist) then
               write (iout,*) ' GETKEY  --  Keyfile Specified'&
   ...
   ! NB Probably do not want to do this and use the 
   !    OPENMP_NUM_THREADS envionment variable.
   do i = 1, nkey
      next = 1
      record = keyline(i)
      call upcase (record)
      ...
      if (keyword(1:15) .eq. 'OPENMP-THREADS ') then
         ...
         if(rank.eq.0) then
           write(iout,*) "Using ",nthread," OpenMP threads from key file."
         end if
      end if
80    continue
   end do
