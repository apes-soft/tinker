...
subroutine redistribute
  ...
  ! this process.
  lmaxbox(1) = maxval(atomlist%pos(1))
  lmaxbox(2) = maxval(atomlist%pos(2))
  lmaxbox(3) = maxval(atomlist%pos(3))

  lminbox(1) = minval(atomlist%pos(1))
  lminbox(2) = minval(atomlist%pos(2))
  lminbox(3) = minval(atomlist%pos(3))


  ! Find the global system extent
  call MPI_Allreduce(lminbox,minbox,3, MPI_DOUBLE_PRECISION, MPI_MIN, &
                    MPI_COMM_WORLD, ierror)
  
  call MPI_Allreduce(lmaxbox,maxbox,3, MPI_DOUBLE_PRECISION, MPI_MAX, &
                    MPI_COMM_WORLD, ierror)
  ...
  do ns=1, nsplits
     ...
     ! local median
     if(mod(dn,2).eq.0) then ! even dn
         localmedian = 0.5*(&
                     atomlist(dn/2)%pos(splits(ns)%splitdir)+&
                     atomlist(dn/2+1)%pos(splits(ns)%splitdir))
     else  ! odd dn
         localmedian = atomlist(int(dn/2+1))%pos(splits(ns)%splitdir)
     endif
     print *,"Process ",rank," has median ",localmedian," split dir ", &
             splits(ns)%splitdir
     ...
     if (splits(ns)%above.eq.1) then
        ! call MPI_Send(buff,count,datatype,neighbor,100,MPI_COMM_WORLD,ierror)
        ! call MPI_Recv(buff,count,datatype,neighbor,101,MPI_COMM_WORLD,ierror)
     else
        ! call MPI_Recv(buff,count,datatype,neighbor,100,MPI_COMM_WORLD,ierror)
        ! call MPI_Send(buff,count,datatype,neighbor,101,MPI_COMM_WORLD,ierror)
     end if
