...
recursive subroutine sortAtoms(dir, start, end)
  ...
  integer, intent(in):: start       ! start index to use for the array
  integer, intent(in):: end         ! end index to use for the array
  real (kind=8):: pivot             ! pivot to be used for the quicksort
  real (kind=8):: findPivot         ! function to find a starting pivot value
  integer:: left, right, midpoint   ! index tracking
  ...
  if(end-start.gt.1) then 
     ...
     if(end-start.gt.limit) then
        ...
        right = end+1   ! scan from the right of the array
        left  = start-1 ! scan from the left of the array
        ...
        do while (left.lt.right)
           ...
           do while (right.ge.start.and.atomlist(right)%pos(dir).gt.pivot)
              right = right-1
           end do
           left = left+1
           ! scan from the left to find an element greater than the pivot
           do while (left.le.end.and.atomlist(left)%pos(dir).lt.pivot)
              left = left+1
           end do
           ! if two elements found swap them
           if(left.lt.right) then
              temp            = atomlist(left)
              atomlist(left)  = atomlist(right)
              atomlist(right) = temp
           endif
        end do
...
subroutine insertionSort(dir,start,end)
  ...
  outter: do i=start+1, end
             ...
             inner: do
                       ...
                       if(atomlist(j)%pos(dir).le.temp%pos(dir)) exit inner
...
function findPivot(dir, left, right)
  ...
  ! gather the sample
  sample = atomlist(left:right:eighth)%pos(dir) 
